...
select * from  [dbo].[TestMultipleColumns]; 
...
SELECT 
    Id,
    Vals,
    LEFT(Vals, PATINDEX('%[^0-9]%', Vals + 'X') - 1) AS ExtractedNumber
FROM GetIntegers
WHERE Vals IS NOT NULL AND PATINDEX('%[0-9]%', Vals) = 1;
...
SELECT
    'rtcfvty34redt' AS OriginalString,
    STRING_AGG(CASE WHEN ch LIKE '[0-9]' THEN '' ELSE ch END, '') AS Letters,
