...
SELECT COALESCE(Email, 'noemail@example.com') AS Email FROM Employees;
...
SELECT EmployeeID,
COALESCE(FirstName, LastName, 'Noma''lum') AS Name 
FROM Employees;
...
SELECT 
    ...
    (SELECT AVG(Price) FROM Products) * 0.1 AS TenPercentOfAverage
FROM Products
