...
SELECT 
    CONCAT(EMPLOYEE_ID, '-', first_name, ' ', last_name) AS EmployeeInfo
FROM employees;
...
select *from DMLTable;
  ...
  SELECT SequenceNumber,
       STRING_AGG(String, ', ') AS EmployeeList
FROM DMLTable
...
SELECT 
    ...
    StudentName,
    Marks,
    SUM(Marks) OVER (ORDER BY StudentID) AS CumulativeTotal
FROM Students;
...
UPDATE Student
SET Score = CASE 
    WHEN StudentName = 'Susan' THEN 10
    WHEN StudentName = 'Tim' THEN 20
    WHEN StudentName = 'Jacob' THEN 30
    WHEN StudentName = 'Earl' THEN 40
    WHEN StudentName = 'Mike' THEN 50
    WHEN StudentName = 'Angie' THEN 60
    WHEN StudentName = 'Jenny' THEN 70
    WHEN StudentName = 'Michelle' THEN 80
    WHEN StudentName = 'Aaron' THEN 90
END;
...
SELECT 
    StudentName,
    Score,
    SUM(Score) OVER (ORDER BY StudentName) AS CumulativeScore
FROM Student;


SELECT s1.Birthday, s1.StudentName, s2.StudentName AS OtherStudent
FROM Student s1
JOIN Student s2 
